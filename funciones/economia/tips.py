...
async def asignar_propina_maestra(bot: BaseBot, user: User, message: str):
    ...
    try:
        ...
        # Diccionarios de Barras y Comisiones
        ...
        target_user = next((u for u, p in res.content if u.username.lower() == target_username), None)
        ...
        # 4. Enviar tips uno por uno
        for bar_item in tip_list:
            await bot.highrise.tip_user(target_user.id, bar_item)
            await asyncio.sleep(0.5) # Pausa pequeña para evitar spam al servidor
        
        await bot.highrise.chat(f"💰 ¡Hecho! He enviado {amount}g a @{target_username}. ✨")

    except Exception as e:
        print(f"Error en tip manual: {e}")

# --- SECCIÓN DE SORTEO AUTOMÁTICO ---
...
async def bucle_tips_automaticos(bot: BaseBot):
    ...
    while tip_sala_activo:
        try:
            ...
            # Candidatos: Nadie que sea Armando ni el bot (buscamos el nombre del bot dinámicamente)
            bot_info = await bot.highrise.get_my_user()
            bot_name = bot_info.content.username.lower()

            candidatos = [u for u in room_users if u.username.lower() not in ["_armando_17_", bot_name]]
            ...
            if candidatos:
                ...
                if wallet.content[0].amount >= 2:
                    ...
                    await bot.highrise.chat(f"🎁 ¡SORTEO! He enviado 1g a @{ganador.username} por estar activo. 🍀")
                else:
                    print("⚠️ Sorteo pausado: El bot se quedó sin oro.")
                    tip_sala_activo = False # Se detiene si no hay fondos
...
async def manejar_control_tips(bot: BaseBot, user: User, message: str):
    global tip_sala_activo
    if user.username.lower() != "_armando_17_": return
